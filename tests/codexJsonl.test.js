...
describe("parseCodexJsonl", () => {
  ...
  it("never substitutes the first prompt for a missing sidebar name", () => {
    const withUser = parseCodexJsonl(
      ri({ type: "message", role: "user", content: [{ type: "input_text", text: "Build a thing" }] })
    );
    expect(withUser.title).toBe("Codex Session (name unavailable)");
    expect(withUser.titleKnown).toBe(false);
  ...
  it("uses supplied names and identifies internal review files", () => {
    const session = parseCodexJsonl([
      L({ type: "session_meta", payload: { id: "review-1", source: { subagent: { other: "guardian" } } } }),
      ri({ type: "message", role: "user", content: [{ text: "A review prompt" }] }),
    ].join("\n"), { title: "An actual name" });
    expect(session.id).toBe("review-1");
    expect(session.internal).toBe(true);
    expect(session.title).toBe("An actual name");
  });
