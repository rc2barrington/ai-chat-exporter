...
export function parseClaudeCodeJsonl(fileContent) {
  ...
  for (const line of fileContent.split("\n")) {
    ...
    try {
      ...
      warnings.push("An unreadable JSONL record was skipped. This saved log may be incomplete.");
      continue;
    }
  }

  const superseded = findSupersededNodes(rows.filter(row => row && !row.isSidechain));
      ...
      if (typeof content === "string") {
        ...
        for (const block of content) {
          ...
          if (block.type === "text" && block.text) {
            ...
            text += "\n\n" + renderAttachment(block, attachments) + "\n\n";
          } else if (block.type === "tool_result") {
            toolResults.push({
              type: "tool_result",
              tool_use_id: block.tool_use_id,
              content: normalizeToolResultContent(block.content, attachments),
      ...
      if (typeof content === "string") {
        ...
        for (const block of content) {
          ...
          if (block.type === "text" && block.text) {
            ...
            blocks.push({ type: "text", text: renderAttachment(block, attachments) });
  ...
  return {
    id: rows.find(row => row?.sessionId)?.sessionId,
    titleKnown: Boolean(customTitle || aiTitle),
    attachments,
...
function renderAttachment(block, attachments) {
  const source = block.source || {};
  const url = source.type === "base64" && source.data && source.media_type
    ? `data:${source.media_type};base64,${source.data}` : source.url;
  return attachmentText({ type: block.type, url, mime: source.media_type, filename: block.title || block.filename }, attachments);
}
...
function normalizeToolResultContent(content, attachments) {
  ...
  if (Array.isArray(content)) {
    return content
      .map((b) => {
        ...
        if (b && (b.type === "image" || b.type === "document")) return renderAttachment(b, attachments);
        return "";
      })
      .join("\n");
  }
