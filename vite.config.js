...
import { EDITION, VERSION } from './src/edition.js'
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), { name: 'edition-stamp', generateBundle() {
      this.emitFile({ type: 'asset', fileName: 'build-info.json', source: JSON.stringify({ edition: EDITION, version: VERSION }) })
    } }, { name: 'local-session-reader', configureServer(server) {
